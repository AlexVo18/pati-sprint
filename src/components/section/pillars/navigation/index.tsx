import type { Pillar, ProductType } from "@/types/pillars.type";
...
const Navigation = ({
  ...
  return (
    <nav
      ...
      role="tablist"
      aria-label="Select a pillar"
    >
      {pillars.map((pillar, index) => (
        <button
          key={pillar.id}
          onClick={() => setActivePillar(activeProduct, index)}
          className={`relative flex items-center gap-4 lg:gap-5 p-4 lg:p-5 rounded-2xl lg:rounded-4xl cursor-pointer text-left overflow-hidden transition-all duration-280 ${
          ...
          style={{
            ...
            backdropFilter:
              currentActivePillarIndex === index ? "none" : "blur(12px)",
        ...
        >
          <span
            className="absolute top-2 left-3 text-[10px] font-medium transition-all duration-280"
            style={{
          ...
          <div
            className="shrink-0 w-11 h-11 lg:w-13 lg:h-13 flex items-center justify-center rounded-xl lg:rounded-[14px] transition-all duration-280"
            style={{
              background:
                currentActivePillarIndex === index
          ...
          >
            <div className="w-[60%] h-[60%]">{pillar.icon}</div>
          </div>

          <div className="flex-1 min-w-0 flex flex-col gap-1">
            <span
              className="text-[15px] lg:text-base font-semibold leading-[1.3] transition-colors duration-280"
              style={{
                fontFamily: "Aeonik, sans-serif",
            ...
            <span
              className="text-xs leading-[1.4] transition-colors duration-280"
              style={{
                fontFamily: "Aeonik, sans-serif",
                color:
                  currentActivePillarIndex === index
          ...
          <div
            className={`shrink-0 transition-all duration-280 ${
              currentActivePillarIndex === index
